...
const Sidebar = ({ onDragStart }) => {
  ...
  const { setAiAssistantOpen, componentSearchQuery, sidebarFilters } = useStore();

  const nodesByCategory = getNodesByCategory();

  // Filter nodes based on search query and filters
  const filteredNodesByCategory = React.useMemo(() => {
    let filtered = { ...nodesByCategory };

    // 1. Filter by Category (if selected in toolbar)
    if (sidebarFilters?.category) {
      const category = sidebarFilters.category;
      filtered = {
        [category]: filtered[category] || [],
      };
    }

    // 2. Filter by Type (if selected in toolbar)
    if (sidebarFilters?.type) {
      const typeFilter = sidebarFilters.type.toLowerCase();
      const newFiltered = {};

      Object.entries(filtered).forEach(([cat, nodes]) => {
        // "custom" type is generic, maybe show all? Or specific?
        // "service", "database", "server" are specific types or keywords
        const matchingNodes = nodes.filter((node) => {
          if (typeFilter === "custom") return true; // Show all for custom for now
          // Check if node id or label matches the type filter
          return (
            node.id.toLowerCase().includes(typeFilter) ||
            node.label.toLowerCase().includes(typeFilter) ||
            node.category.toLowerCase() === typeFilter
          );
        });

        if (matchingNodes.length > 0) {
          newFiltered[cat] = matchingNodes;
        }
      });
    ...
    if (componentSearchQuery) {
      ...
      Object.entries(filtered).forEach(([category, nodes]) => {
        ...
        if (matchingNodes.length > 0) {
          newFiltered[category] = matchingNodes;
        }
      });
      filtered = newFiltered;
    }
    ...
    return filtered;
  }, [nodesByCategory, componentSearchQuery, sidebarFilters]);

  // Auto-expand categories when searching or filtering
  React.useEffect(() => {
    if (componentSearchQuery || sidebarFilters?.category || sidebarFilters?.type) {
      setExpandedCategories(
        Object.keys(filteredNodesByCategory).reduce(
          (acc, category) => ({ ...acc, [category]: true }),
          {}
        )
      );
    }
  }, [componentSearchQuery, sidebarFilters, filteredNodesByCategory]);
